...
class Tools:
    ...
    def _proto_to_dict(self, message: Optional[Message]) -> Any:
        ...
        try:
            ...
            try:
                return MessageToDict(
                    message,
                    including_default_value_fields=True,  # type: ignore[call-arg]
                    preserving_proto_field_name=True,
                    use_integers_for_enums=False,
                )
            except TypeError:
    ...
    def _field_type_name(self, field: Any) -> str:
        type_map = {
    ...
    def _resolve_proto_field(
        ...
        for part in parts[:-1]:
            ...
            if field.type != FieldDescriptor.TYPE_MESSAGE or field.is_repeated:
    ...
    def _coerce_scalar(self, field: Any, value: Any) -> Any:
    ...
    def _rf_int(self, value: Any, default: Optional[int] = None) -> Optional[int]:
        try:
            if value is None:
                return default
            return int(value)
        ...
        def run() -> Dict[str, Any]:
            ...
            def op(iface: TCPInterface) -> Dict[str, Any]:
                ...
                if sort_key == "last_heard":
                    ...
                    def _hops_sort_key(row: Dict[str, Any]) -> int:
                        hops_away = row.get("hopsAway")
                        return int(hops_away) if hops_away is not None else 999

                    rows.sort(key=_hops_sort_key)
    ...
    async def get_mesh_health(
        ...
        def run() -> Dict[str, Any]:
            ...
            def op(iface: TCPInterface) -> Dict[str, Any]:
                ...
                def _snr_sort_key(n: Dict[str, Any]) -> float:
                    snr = n.get("snr")
                    return float(snr) if snr is not None else 0.0
    ...
    def _topology_render_html(self, graph: "nx.Graph", local_label: str, summary: Dict[str, Any]) -> str:
        ...
        net = Network(
            ...
            width="100%",
            bgcolor="#111318",
            font_color="#e6e6e6",  # type: ignore[arg-type]  # pyvis param is untyped (default False) but documented as accepting a color str
            ...
            json.dumps(
                {
                    ...
                    "interaction": {"hover": True, "dragNodes": False, "zoomView": True, "tooltipDelay": 120},
                    "edges": {"smooth": False},
                }
            )
        )
...
"""
        ...
        legend = f"""
        <div style="background:var(--mesh-legend-bg);color:var(--mesh-font);
            border-top:1px solid var(--mesh-legend-border);padding:10px 14px;
            font:12px/1.5 -apple-system,Segoe UI,sans-serif;
            display:flex;flex-wrap:wrap;column-gap:20px;row-gap:4px;align-items:baseline;">
        <div style="font-weight:600;">Mesh topology</div>
        ...
        <div style="color:var(--mesh-legend-muted);">Dashed = hop-count only or MQTT, not a measured RF link</div>
        <div style="color:var(--mesh-legend-muted);">Concentric rings = hop count (innermost = directly heard)</div>
        </div>
        """
    ...
    async def get_mesh_topology(
        ...
        count, and the diagram stays pan/zoom/hover interactive (node positions
        are fixed so ring position always reflects hop count at a glance). The JSON
            ...
            try:
                ...
                def run() -> Tuple[Dict[str, Any], Optional[str]]:
                    def op(iface: TCPInterface) -> Tuple[Dict[str, Any], Optional[str]]:
                        ...
                        def _topology_sort_key(n: Dict[str, Any]) -> Tuple[int, float]:
                            hops_away = n.get("hopsAway")
                            snr = n.get("snr")
                            return (
                                int(hops_away) if hops_away is not None else 99,
                                -float(snr) if snr is not None else 0.0,
                            )

                        active.sort(key=_topology_sort_key)
            ...
            def op(iface: TCPInterface) -> Dict[str, Any]:
                ...
                if receiver_sensitivity_dbm is not None:
                    ...
                    sensitivity = self._rf_receiver_sensitivity_dbm(
                        ...
                        self._rf_int(modem.get("spreadingFactor")),
                    )
    ...
    async def rf_analyse_mesh(
        ...
        def run() -> Dict[str, Any]:
            ...
            def op(iface: TCPInterface) -> Dict[str, Any]:
                ...
                modem = context.get("modem") or {}
                sensitivity = self._rf_receiver_sensitivity_dbm(self._rf_float(modem.get("bandwidthKHz")), self._rf_int(modem.get("spreadingFactor")))
    ...
    async def request_telemetry(
        ...
        def run() -> Dict[str, Any]:
            ...
            def op(iface: TCPInterface) -> Dict[str, Any]:
                ...
                def on_response(p: Dict[str, Any]) -> None:
                    ...
                    if portnum == "TELEMETRY_APP" or portnum == portnums_pb2.PortNum.TELEMETRY_APP:
                        ...
                        captured["responseFrom"] = p.get("from")
                        captured["responseTo"] = p.get("to")
                    else:
                        captured["responsePacket"] = self._clean_data(p)
