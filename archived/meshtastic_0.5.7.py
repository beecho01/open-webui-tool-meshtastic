...
class Tools:
    ...
    RF_REGION_PROFILES: Dict[str, Dict[str, Any]] = {
        ...
        "PH_915": {
            ...
            "reg_basis": "EIRP",
            "reg_limit": 24.0,
            "note": "Firmware source notes that external antennas are not allowed for this regional profile.",
        },
        "KZ_433": {
            "start": 433.075,
            "end": 434.775,
        ...
        "KZ_863": {
            ...
            "reg_basis": "EIRP",
            "reg_limit": 14.0,
            "note": "Firmware's regional TX ceiling is 30 dBm, but its regulatory source comment states <25 mW EIRP; the planner therefore uses 14 dBm EIRP for the radiated-power check.",
        },
        "NP_865": {
            "start": 865.0,
            "end": 868.0,
    ...
    RF_MODEM_PRESETS: Dict[str, Dict[str, Any]] = {
        "SHORT_TURBO": {
            "bandwidth_khz": 500.0,
            "wide_bandwidth_khz": 1625.0,
            "sf": 7,
            "cr": 5,
            "documented_link_budget_db": 140.0,
            "channel_name": "ShortTurbo",
        },
        "SHORT_FAST": {
            "bandwidth_khz": 250.0,
            "wide_bandwidth_khz": 812.5,
            "sf": 7,
            "cr": 5,
            "documented_link_budget_db": 143.0,
            "channel_name": "ShortFast",
        },
        "SHORT_SLOW": {
            "bandwidth_khz": 250.0,
            "wide_bandwidth_khz": 812.5,
            "sf": 8,
            "cr": 5,
            "documented_link_budget_db": 145.5,
            "channel_name": "ShortSlow",
        },
        "MEDIUM_FAST": {
            "bandwidth_khz": 250.0,
            "wide_bandwidth_khz": 812.5,
            "sf": 9,
            "cr": 5,
            "documented_link_budget_db": 148.0,
        ...
        "MEDIUM_SLOW": {
            ...
            "sf": 10,
            "cr": 5,
            "documented_link_budget_db": 150.5,
            "channel_name": "MediumSlow",
        },
        "MEDIUM_TURBO": {
            "bandwidth_khz": 500.0,
            "wide_bandwidth_khz": 1625.0,
        ...
        "LONG_TURBO": {
            ...
            "sf": 11,
            "cr": 8,
            "documented_link_budget_db": 150.0,
            "channel_name": "LongTurbo",
        },
        "LONG_FAST": {
            "bandwidth_khz": 250.0,
            "wide_bandwidth_khz": 812.5,
            "sf": 11,
            "cr": 5,
            "documented_link_budget_db": 153.0,
            "channel_name": "LongFast",
        },
        "LONG_MODERATE": {
            "bandwidth_khz": 125.0,
            "wide_bandwidth_khz": 406.25,
            "sf": 11,
            "cr": 8,
            "documented_link_budget_db": 156.0,
            "channel_name": "LongMod",
        },
        "LONG_SLOW": {
            "bandwidth_khz": 125.0,
            "wide_bandwidth_khz": 406.25,
            "sf": 12,
            "cr": 8,
            "documented_link_budget_db": 158.5,
            "channel_name": "LongSlow",
            "deprecated": True,
        },
        "LITE_FAST": {
            "bandwidth_khz": 125.0,
            "sf": 9,
            "cr": 5,
            "channel_name": "LiteFast",
        },
        "LITE_SLOW": {
            "bandwidth_khz": 125.0,
            "sf": 10,
            "cr": 5,
            "channel_name": "LiteSlow",
        },
        "NARROW_FAST": {
            "bandwidth_khz": 62.5,
            "sf": 7,
            "cr": 6,
            "channel_name": "NarrowFast",
        },
        "NARROW_SLOW": {
            "bandwidth_khz": 62.5,
            "sf": 8,
            "cr": 6,
            "channel_name": "NarrowSlow",
        },
        "TINY_FAST": {
            "bandwidth_khz": 15.6,
            "sf": 7,
            "cr": 5,
            "channel_name": "TinyFast",
        },
        "TINY_SLOW": {
            "bandwidth_khz": 15.6,
            "sf": 8,
            "cr": 6,
            "channel_name": "TinySlow",
        },
    }
    ...
    def _preflight(self) -> None:
        ...
        if not host:
            raise ValueError(
                "Meshtastic host is empty. Set the tool's host Valve first."
            )
    ...
    def _with_interface(
        self, func: Callable[[TCPInterface], Any], no_nodes: bool = False
    ) -> Any:
        self._preflight()
    ...
    def _find_node_entry(
        self, iface: TCPInterface, node_ref: str
        ...
        for key, node in nodes.items():
            ...
            if any(
                candidate.casefold() == needle for candidate in candidates if candidate
            ):
    ...
    def _get_config_message(
        self, iface: TCPInterface, section: str
    ) -> Tuple[str, Message]:
    ...
    def _describe_message(
        self, message: Message, prefix: str = ""
    ) -> List[Dict[str, Any]]:
        ...
        if field.type == FieldDescriptor.TYPE_ENUM:
            if isinstance(value, int):
                if field.enum_type.values_by_number.get(value) is None:
                    raise ValueError(
                        f"{value} is not a valid enum value for {field.name}"
                    )
                return value
            text = str(value).strip()
            if text.lstrip("-").isdigit():
                ...
                if field.enum_type.values_by_number.get(number) is None:
                    raise ValueError(
                        f"{number} is not a valid enum value for {field.name}"
                    )
                return number
            target = text.upper()
            ...
            raise ValueError(
                f"Invalid enum '{value}' for {field.name}. Valid values: {valid}"
            )
    ...
    def _set_proto_value(
        self, message: Message, field_path: str, value_text: str
    ) -> Dict[str, Any]:
    ...
    def _validate_write_permission(self, section: str, field_path: str) -> None:
        if not self.valves.allow_config_writes:
            raise PermissionError(
                "Configuration writes are disabled in this tool's Valves"
            )
    ...
    def _config_preview_sync(
        self, section: str, field_path: str, value: str
    ) -> Dict[str, Any]:
        def op(iface: TCPInterface) -> Dict[str, Any]:
            ...
            proposal["requiresSensitiveWritePermission"] = bool(
                section.strip().lower() == "security"
                or self._is_sensitive_key(field_path)
            )
    ...
    def _parse_batch(self, changes_json: str) -> List[Dict[str, str]]:
        ...
        if len(raw) > 30:
            raise ValueError(
                "A maximum of 30 config changes can be applied in one batch"
            )
    ...
    def _rf_modem_profile(
        self, lora: Dict[str, Any], wide_lora: bool = False
    ) -> Dict[str, Any]:
        ...
        if use_preset and preset_name in self.RF_MODEM_PRESETS:
            ...
            if 5 <= live_cr <= 8 and live_cr > int(result.get("cr") or 0):
                result["cr"] = live_cr
                result["coding_rate_source"] = (
                    "live higher custom coding_rate over preset"
                )
            else:
                result["coding_rate_source"] = "preset"
            result.update(
                {
                    "source": "Meshtastic preset",
                    "preset": preset_name,
                    "usePreset": True,
                }
            )
        ...
        if use_preset:
            ...
            warnings.append(
                "Custom LoRa settings did not expose valid bandwidth, spreading factor and coding rate values."
            )
        return {
    ...
    def _rf_node_geo(self, node: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if lon is None:
            raw = self._rf_float(
                position.get("longitudeI", position.get("longitude_i"))
            )
        ...
        return {
            ...
            "reportedAltitudeSource": (
                f"NodeDB position.{altitude_key}" if altitude_key else None
            ),
    ...
    def _rf_hardware_context(self, iface: TCPInterface) -> Dict[str, Any]:
        ...
        return {
            ...
            "actualConductedPowerKnown": float(
                self.valves.rf_local_measured_tx_power_dbm
            )
            > -9000.0,
            "measuredConductedPowerDbm": (
                float(self.valves.rf_local_measured_tx_power_dbm)
                if float(self.valves.rf_local_measured_tx_power_dbm) > -9000.0
                else None
            ),
            "confidence": (
                "measured/user-verified"
                if float(self.valves.rf_local_measured_tx_power_dbm) > -9000.0
                else ("family hint only" if hint else "unknown")
            ),
            "note": hint.get("note")
            or "No hardware-specific conducted-power model is encoded. Live tx_power is a configured/system target, not a bench measurement.",
        }
    ...
    def _rf_local_installation(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "antennaGainDbi": (
                float(self.valves.rf_local_antenna_gain_dbi) if enabled else None
            ),
            "cableLossDb": (
                float(self.valves.rf_local_cable_loss_db) if enabled else None
            ),
            "antennaHeightMAboveGround": (
                float(self.valves.rf_local_antenna_height_m_agl) if enabled else None
            ),
            "groundElevationMAsl": (
                float(self.valves.rf_local_ground_elevation_m_asl)
                if enabled
                and float(self.valves.rf_local_ground_elevation_m_asl) > -9000.0
                else None
            ),
            "measuredTxPowerDbm": (
                float(self.valves.rf_local_measured_tx_power_dbm)
                if enabled
                and float(self.valves.rf_local_measured_tx_power_dbm) > -9000.0
                else None
            ),
    ...
    def _rf_remote_profiles(self) -> Dict[str, Dict[str, Any]]:
        ...
        if not isinstance(data, dict):
            raise ValueError(
                "rf_remote_profiles_json must be a JSON object keyed by node ID/name"
            )
    ...
    def _rf_remote_profile_for(
        self, node_key: str, node: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _rf_resolve_local_assumption(
        self, explicit: float, profile_key: str, fallback: float = 0.0
    ) -> Tuple[float, str]:
    ...
    def _rf_resolve_remote_assumption(
        self,
        explicit: float,
    ...
    def _rf_terrain_sampling_plan(
        ...
        mode = (
            str(self.valves.rf_terrain_sampling_mode or "adaptive").strip().casefold()
        )
        ...
        actual_spacing_m = (
            distance_m / max(1, sample_count - 1) if distance_m > 0 else 0.0
        )
    ...
    def _rf_fetch_terrain_profile(
        ...
        if not self.valves.rf_allow_external_terrain_requests:
            raise PermissionError(
                "Terrain requests are disabled. Enable rf_allow_external_terrain_requests after choosing a trusted public or self-hosted elevation service."
            )
        ...
        dataset = (
            (self.valves.rf_terrain_dataset or "srtm30m,mapzen,etopo1")
            .strip()
            .strip("/")
        )
        if not dataset:
        ...
        request = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "MeshOps-RF/0.4.1",
            },
            method="POST",
        )
        try:
            with urllib.request.urlopen(
                request, timeout=float(self.valves.rf_terrain_timeout_seconds)
            ) as response:
        ...
        if data.get("status") != "OK":
            raise RuntimeError(
                f"Terrain API returned {data.get('status')}: {data.get('error')}"
            )
        points = []
        ...
        if len(points) < 2 or any(p.get("groundElevationMAsl") is None for p in points):
            raise RuntimeError(
                "Terrain API returned incomplete elevation coverage for this path"
            )
        return {
            "provider": base,
    ...
    def _rf_endpoint_antenna_asl(
        ...
        ground = (
            explicit_ground_asl
            if explicit_ground_asl is not None
            else terrain_ground_asl
        )
        if ground is not None and antenna_height_agl > 0:
            return (
                float(ground) + antenna_height_agl,
                "ground elevation + supplied antenna AGL",
                float(ground),
            )
        ...
        return (
            None,
            "unknown: need terrain/ground elevation plus antenna AGL, or a reported device altitude",
            ground,
        )
    ...
    def _rf_terrain_analysis(
        ...
        for index, point in enumerate(points):
            ...
            earth_bulge = (
                (d1 * d2) / (2.0 * earth_radius_m) if d1 > 0 and d2 > 0 else 0.0
            )
        ...
        if h > 0:
            ...
            if v > -0.78:
                diffraction = 6.9 + 20.0 * math.log10(
        ...
        return {
            ...
            "fresnel60PercentClear": float(worst_fresnel["fresnel60ClearanceMarginM"])
            >= 0.0,
            "minimumLineOfSightClearanceM": worst_los["lineOfSightClearanceM"],
            "minimum60PercentFresnelMarginM": worst_fresnel[
                "fresnel60ClearanceMarginM"
            ],
            "worstLineOfSightPoint": worst_los,
            "worstFresnelPoint": worst_fresnel,
            "singleKnifeEdgeLossEstimateDb": round(diffraction, 2),
            "sampleCount": len(rows),
            "endpointGroundElevationsMAsl": [
                points[0]["groundElevationMAsl"],
                points[-1]["groundElevationMAsl"],
            ],
            "profile": rows,
            "limitations": [
    ...
    def _rf_noise_floor_dbm(
        self, bandwidth_khz: Optional[float], noise_figure_db: float = 6.0
    ) -> Optional[float]:
    ...
    def _rf_confidence(
        ...
        *,
        ...
        return {
            "endpointGeometry": (
                "high" if terrain else "medium" if local_profile else "low"
            ),
            "localRfChain": (
                "high"
                if actual_tx and local_profile
                else "medium" if local_profile else "low"
            ),
            "remoteRfChain": "medium" if remote_profile else "low",
            "propagationPrediction": (
                "medium-high"
                if terrain and direct_observation
                else "medium" if terrain else "low"
            ),
    ...
    def _rf_context_from_iface(self, iface: TCPInterface) -> Dict[str, Any]:
        ...
        if region_name == "UNSET":
            warnings.append(
                "LoRa region is UNSET. Do not use the regional power/frequency defaults as permission to transmit."
            )
        ...
        slots = None
        slot_width_mhz = None
        channel_name = self._rf_primary_channel_name(
            iface, str(modem.get("preset") or "LONG_FAST")
        )
        ...
        if region.get("licensed_only"):
            warnings.append(
                "This is an amateur/licensed region profile. Licence privileges and the applicable band plan take precedence over generic planner limits."
            )
        if region.get("note"):
            warnings.append(str(region["note"]))
        if region.get("frequency_switching"):
            warnings.append(
                "This region uses frequency switching; a single calculated centre frequency does not describe all transmissions."
            )
        ...
        return {
            ...
            "frequencySlotWidthMHz": (
                round(slot_width_mhz, 6) if slot_width_mhz is not None else None
            ),
    ...
    def _rf_receiver_sensitivity_dbm(
        ...
        if not bandwidth_khz or not sf or sf not in self.RF_SNR_THRESHOLDS_DB:
            return None
        noise_floor = (
            -174.0 + 10.0 * math.log10(bandwidth_khz * 1000.0) + noise_figure_db
        )
            ...
            result.update(
                {
                    ...
                    "status": (
                        "within_planner_limit"
                        if headroom >= -1e-9
                        else "exceeds_planner_limit"
                    ),
                }
            )
        return result
    ...
    def _rf_geometry(
        ...
        combined = (
            (tx_horizon + rx_horizon)
            if tx_horizon is not None and rx_horizon is not None
            else None
        )
        return {
            "firstFresnelRadiusAtMidpointM": round(fresnel, 2),
            "recommended60PercentFresnelClearanceM": round(fresnel * 0.6, 2),
            "localRadioHorizonContributionKm": (
                round(tx_horizon, 2) if tx_horizon is not None else None
            ),
            "remoteRadioHorizonContributionKm": (
                round(rx_horizon, 2) if rx_horizon is not None else None
            ),
            "approximateCombinedRadioHorizonKm": (
                round(combined, 2) if combined is not None else None
            ),
            "approximateRadioHorizonKm": (
                round(combined, 2) if combined is not None else None
            ),
            "radioHorizonModel": (
                "4/3-earth smooth-Earth approximation using antenna heights AGL"
    ...
    def _rf_validate_numeric(
        self, name: str, value: float, minimum: Optional[float] = None
    ) -> float:
        numeric = float(value)
        if not math.isfinite(numeric):
                ...
                "result": {
                    ...
                    "capabilities": {
                        ...
                        "rfTerrainEffectiveMaxSamples": (
                            self.valves.rf_terrain_max_samples
                            if self.valves.rf_terrain_self_hosted
        ...
        def run() -> Dict[str, Any]:
            def op(iface: TCPInterface) -> Dict[str, Any]:
                ...
                return {
                    ...
                    "myInfo": self._clean_data(iface.myInfo),
                    "localNode": (
                        self._augment_node(my_node)
                        if isinstance(my_node, dict)
                        else self._clean_data(my_node)
                    ),
        ...
        def run() -> Dict[str, Any]:
            ...
            def op(iface: TCPInterface) -> Dict[str, Any]:
                ...
                for key, node in (iface.nodes or {}).items():
                    if (
                        not include_self
                        and my_num is not None
                        and node.get("num") == my_num
                    ):
                        continue
                    if active_within_minutes > 0:
                ...
                def battery(row: Dict[str, Any]) -> float:
                    return float(
                        (row.get("deviceMetrics") or {}).get("batteryLevel") or -1
                    )
                ...
                if sort_key == "last_heard":
                    ...
                    rows.sort(
                        key=lambda r: int(
                            r.get("hopsAway") if r.get("hopsAway") is not None else 999
                        )
                    )
    ...
    async def search_nodes(self, query: str, __event_emitter__=None) -> str:
        ...
        return await self._run_sync(
            run, __event_emitter__, "Searching NodeDB…", "Node search complete"
        )
    ...
    async def get_node(self, node: str, __event_emitter__=None) -> str:
        ...
        return await self._run_sync(
            run, __event_emitter__, "Reading node details…", "Node details loaded"
        )
    ...
    async def get_mesh_health(
        ...
        def run() -> Dict[str, Any]:
            ...
            def op(iface: TCPInterface) -> Dict[str, Any]:
                ...
                hops = [
                    int(n["hopsAway"]) for n in active if n.get("hopsAway") is not None
                ]
                ...
                return {
                    ...
                    "localMetrics": self._clean_data(
                        my_info.get("deviceMetrics") or {}
                    ),
                        ...
                        self._clean_data(
                            {
                                ...
                                "lastHeardIsoUtc": self._timestamp_iso(
                                    n.get("lastHeard")
                                ),
                            }
                        )
    ...
    def _topology_render_html(
        self, graph: "nx.Graph", local_label: str, summary: Dict[str, Any]
    ) -> str:
        ...
        for ring, ids in sorted(rings.items()):
            ...
            ring_specs = [[ring_spacing * (r + 1), r] for r in sorted(rings)]
            count = len(ids)
        ...
        net = Network(
            ...
            cdn_resources=(
                "in_line"
                if str(self.valves.topology_vis_js_source).strip().casefold()
                == "in_line"
                else "remote"
            ),
        )
        ...
        for node_id, attrs in graph.nodes(data=True):
            ...
            node_color = "#9aa0a6"
            if (
                attrs.get("hopsAway") == 0
                and not attrs.get("viaMqtt")
                and snr is not None
            ):
        ...
        for u, v, attrs in graph.edges(data=True):
            if attrs.get("measured"):
                ...
                net.add_edge(
                    ...
                    width=style["width"],
                    color=style["color"],
                    title=f"{attrs.get('snr')} dB (direct, measured)",
                )
                ...
                net.add_edge(
                    ...
                    color="#5aa9e6",
                    dashes=True,
                    title="Heard via MQTT (not an RF link)",
                )
            else:
                hops = attrs.get("hopsAway")
                net.add_edge(
                    ...
                    color="#555a63",
                    dashes=True,
                    title=f"{hops} hops away - relay path unknown, run traceroute for the real path",
                )
        ...
        ring_hook = (
            ...
            "  var spec = " + json.dumps(ring_specs) + ";\n"
            "  var dark = document.documentElement.getAttribute('data-mesh-theme') !== 'light';\n"
            "  ctx.save();\n"
            "  ctx.strokeStyle = dark ? 'rgba(255,255,255,0.16)' : 'rgba(0,0,0,0.14)';\n"
            "  ctx.lineWidth = 1.5;\n"
            "  for (var i = 0; i < spec.length; i++) {\n"
            "    ctx.beginPath();\n"
            "    ctx.arc(0, 0, spec[i][0], 0, Math.PI * 2);\n"
            "    ctx.stroke();\n"
            "  }\n"
            "  ctx.restore();\n"
            "});\n"
            "if (window.applyMeshTheme) window.applyMeshTheme();\n"
        )
        html = html.replace(
        ...
        # Theme detection: follow Open WebUI's theme when the embed iframe
        # can read the parent (iframeSandboxAllowSameOrigin enabled), else
        # fall back to prefers-color-scheme (OS). Sets data-mesh-theme.
        theme_script = """
<script>
(function () {
  function osDark() { try { return window.matchMedia('(prefers-color-scheme: dark)').matches; } catch (e) { return false; } }
  function detect() {
    var t = osDark() ? 'dark' : 'light';
    try {
      var p = parent.document.documentElement;
      if (p.classList.contains('dark') || p.classList.contains('oled-dark') || p.getAttribute('data-theme') === 'dark' || p.getAttribute('data-theme') === 'oled') t = 'dark';
      else if (p.classList.contains('light') || p.getAttribute('data-theme') === 'light') t = 'light';
      else {
        var s = parent.localStorage.getItem('theme') || '';
        if (s.indexOf('oled') > -1 || s.indexOf('dark') > -1) t = 'dark';
        else if (s === 'light' || s === 'her') t = 'light';
        else if (s === 'system') t = osDark() ? 'dark' : 'light';
      }
    } catch (e) {}
    return t;
  }
  function applyTheme() {
    var t = detect();
    document.documentElement.setAttribute('data-mesh-theme', t);
    if (typeof network !== 'undefined' && network && network.body && network.body.data) {
      var fc = t === 'light' ? '#1f2328' : '#e6e6e6';
      try {
        var ds = network.body.data.nodes;
        ds.get().forEach(function (n) { ds.update({id: n.id, font: {color: fc}}); });
        network.redraw();
      } catch (e) {}
    }
  }
  window.applyMeshTheme = applyTheme;
  applyTheme();
  try { window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', applyTheme); } catch (e) {}
  try {
    var p = parent.document.documentElement;
    new parent.MutationObserver(applyTheme).observe(p, { attributes: true, attributeFilter: ['class', 'data-theme'] });
  } catch (e) {}
  try { window.addEventListener('storage', function (e) { if (e.key === 'theme' || e.key === null) applyTheme(); }); } catch (e) {}
})();
</script>
"""

        # Strip pyvis's Bootstrap card chrome and make the diagram
        # theme-aware. --bs-light is defined by the Bootstrap CSS pyvis
        # loads in the iframe, so it resolves inside the embed.
        chrome_strip = """
<style>
:root { --mesh-bg: #111318; --mesh-font: #e6e6e6; --mesh-legend-bg: rgba(0,0,0,0.78); --mesh-legend-border: #4a4f5a; --mesh-legend-muted: #9aa0a6; }
:root[data-mesh-theme="light"] { --mesh-bg: var(--bs-light); --mesh-font: #1f2328; --mesh-legend-bg: rgba(255,255,255,0.92); --mesh-legend-border: #d0d7de; --mesh-legend-muted: #6a6f76; }
html, body { margin: 0 !important; padding: 0 !important; background: var(--mesh-bg) !important; }
.card { border: none !important; background: transparent !important; margin: 0 !important; box-shadow: none !important; }
.card-header { display: none !important; }
.card-body { padding: 0 !important; background: transparent !important; border: none !important; }
#mynetwork { height: 600px !important; border: none !important; background: var(--mesh-bg) !important; }
</style>
"""
        html = html.replace("</head>", theme_script + chrome_strip + "</head>", 1)
        legend = f"""
        <div style="position:absolute;left:12px;bottom:12px;background:var(--mesh-legend-bg);
            color:var(--mesh-font);border:1px solid var(--mesh-legend-border);border-radius:8px;padding:10px 14px;
            font:12px/1.5 -apple-system,Segoe UI,sans-serif;max-width:260px;z-index:9999;
        ...
        <div style="margin-top:4px;color:var(--mesh-legend-muted);">Dashed = hop-count only or MQTT, not a measured RF link</div>
        <div style="margin-top:4px;color:var(--mesh-legend-muted);">Concentric rings = hop count (innermost = directly heard)</div>
        </div>
        """
    ...
    async def get_mesh_topology(
        ...
        delivers an interactive vis.js diagram to the chat as an inline Open WebUI
        embed (a sandboxed, script-enabled iframe): edge thickness and colour
        reflect measured signal strength, nodes are laid out in rings by hop
        count, and the diagram stays pan/zoom/drag/hover interactive. The JSON
        summary in the tool result is what you (the model) see; you do not need
        to reproduce the diagram - it is shown to the user automatically. Just
        describe the mesh in plain language based on the summary.
        ...
        async with self._operation_lock:
            await self._status(
                __event_emitter__, "Building Meshtastic mesh topology…", False
            )
            try:
                if active_within_minutes < 1 or active_within_minutes > 10080:
                    raise ValueError(
                        "active_within_minutes must be between 1 and 10080"
                    )
                ...
                def run() -> Tuple[Dict[str, Any], Optional[str]]:
                    def op(iface: TCPInterface) -> Tuple[Dict[str, Any], Optional[str]]:
                        ...
                        local_label = str(
                            my_user.get("longName")
                            or my_user.get("shortName")
                            or "Local node"
                        )

                        nodes = list((iface.nodes or {}).values())
                        peers = [
                            n for n in nodes if my_num is None or n.get("num") != my_num
                        ]
                        active = [
                            n
                            for n in peers
                            if int(n.get("lastHeard") or 0) > 0
                            and now - int(n.get("lastHeard") or 0)
                        ...
                        for entry in active:
                            ...
                            label = str(
                                user.get("shortName") or user.get("longName") or node_id
                            )
                            hops = entry.get("hopsAway")
                            snr = entry.get("snr")
                            via_mqtt = bool(entry.get("viaMqtt"))
                            battery = (entry.get("deviceMetrics") or {}).get(
                                "batteryLevel"
                            )
                            ...
                            link_type = (
                                "direct_measured"
                                if is_direct
                                else ("mqtt" if via_mqtt else "hop_count_only")
                            )
                            links.append(
                                {
                                    "id": user.get("id"),
                        ...
                        direct_snrs = [
                            float(l["snrDb"])
                            for l in links
                            if l["linkType"] == "direct_measured"
                        ]
                        summary = {
                            "windowMinutes": active_within_minutes,
                            "nodeCount": len(active),
                            "directMeasuredLinks": len(direct_snrs),
                            "hopCountOnlyLinks": sum(
                                1 for l in links if l["linkType"] == "hop_count_only"
                            ),
                            "mqttOnlyLinks": sum(
                                1 for l in links if l["linkType"] == "mqtt"
                            ),
                            "strongestDirectSnrDb": (
                                max(direct_snrs) if direct_snrs else None
                            ),
                            "weakestDirectSnrDb": (
                                min(direct_snrs) if direct_snrs else None
                            ),
                        ...
                        if self.valves.topology_render_interactive and active:
                            html = self._topology_render_html(
                                graph, local_label, summary
                            )
                ...
                await self._status(
                    __event_emitter__, "Mesh topology diagram ready", True
                )

                result_payload = self._json(
                    {"ok": True, "result": summary, "diagramRendered": bool(html)}
                )

                if html:
                    # Deliver the interactive diagram as an inline Open WebUI embed.
                    # Returning (HTMLResponse, result_context) lets the Open WebUI
                    # tool middleware extract the page and emit it as an `embeds`
                    # event, which the frontend renders via FullHeightIframe in a
                    # sandboxed, script-enabled iframe (stored on message.embeds).
                    # This survives the message-content overwrite that happens at
                    # response completion and persists across chat reloads - unlike
                    # the previous __event_emitter__ "message" approach, which
                    # appended a ```html fence to message.content and was discarded
                    # by `message.content = getOutputText(message.output)`.
                    return (
                        HTMLResponse(
                            content=html,
                ...
                await self._status(__event_emitter__, "Mesh topology failed", True)
                return self._json(
                    {"ok": False, "error": type(exc).__name__, "details": str(exc)}
                )
    ...
    async def rf_analyse_link(
        ...
        def run() -> Dict[str, Any]:
            explicit_distance = self._rf_validate_numeric(
                "distance_km", distance_km, 0.0
            )
            explicit_frequency = self._rf_validate_numeric(
                "frequency_mhz", frequency_mhz, 0.0
            )
            fade = self._rf_validate_numeric("fade_margin_db", fade_margin_db, 0.0)
            extra_loss = self._rf_validate_numeric(
                "extra_path_loss_db", extra_path_loss_db, 0.0
            )
            ...
            def op(iface: TCPInterface) -> Dict[str, Any]:
                ...
                if node.strip():
                    ...
                    if local_geo and remote_geo:
                        a = (
                            float(local_geo["latitude"]),
                            float(local_geo["longitude"]),
                        )
                        b = (
                            float(remote_geo["latitude"]),
                            float(remote_geo["longitude"]),
                        )
                        derived_distance = self._rf_haversine_km(a, b)
                        target["bearingDegreesFromLocal"] = round(
                            self._rf_initial_bearing_deg(a, b), 1
                        )
                        target["bearingSectorFromLocal"] = self._rf_bearing_sector(
                            float(target["bearingDegreesFromLocal"])
                        )
                    if target["viaMqtt"]:
                        warnings.append(
                            "Target was learned via MQTT; stored SNR/hops are not a direct RF observation."
                        )
                    hops = target.get("hopsAway")
                    if hops not in (None, 0):
                        warnings.append(
                            "Target is multi-hop. Endpoint distance is not the distance of any single RF hop, so stored SNR cannot calibrate this end-to-end path."
                        )

                distance = (
                    explicit_distance if explicit_distance > 0 else derived_distance
                )
                if distance is None or distance <= 0:
                    raise ValueError(
                        "Provide distance_km > 0, or name a NodeDB node where both endpoint positions are available"
                    )

                frequency = (
                    explicit_frequency
                    if explicit_frequency > 0
                    else self._rf_float(context.get("frequencyMHz"))
                )
                if frequency is None or frequency <= 0:
                    raise ValueError(
                        "The live radio frequency could not be derived. Provide frequency_mhz explicitly"
                    )

                # Resolve local and remote RF-chain inputs without silently converting assumptions into facts.
                tx_gain, tx_gain_source = self._rf_resolve_local_assumption(
                    float(tx_antenna_gain_dbi), "antennaGainDbi"
                )
                tx_loss, tx_loss_source = self._rf_resolve_local_assumption(
                    float(tx_cable_loss_db), "cableLossDb"
                )
                tx_height, tx_height_source = self._rf_resolve_local_assumption(
                    float(tx_height_m), "antennaHeightMAboveGround"
                )
                rx_gain, rx_gain_source = self._rf_resolve_remote_assumption(
                    float(rx_antenna_gain_dbi),
                    remote_profile,
                    ("antenna_gain_dbi", "antennaGainDbi"),
                )
                rx_loss, rx_loss_source = self._rf_resolve_remote_assumption(
                    float(rx_cable_loss_db),
                    remote_profile,
                    ("cable_loss_db", "cableLossDb"),
                )
                rx_height, rx_height_source = self._rf_resolve_remote_assumption(
                    float(rx_height_m),
                    remote_profile,
                    ("antenna_height_m_agl", "antennaHeightMAboveGround"),
                )

                tx_context = context.get("tx") or {}
                if actual_tx_power_dbm is not None:
                    tx_power = self._rf_validate_numeric(
                    ...
                    if tx_power is None:
                        raise ValueError(
                            "TX power could not be derived. Provide actual_tx_power_dbm explicitly"
                        )
                    tx_source = str(
                        tx_context.get("planningTxPowerSource") or "live configuration"
                    )
                    actual_tx_known = bool(tx_context.get("hardwareActualTxPowerKnown"))
                    if not actual_tx_known:
                        warnings.append(
                            "TX power is a configured/system target rather than a measured conducted output. Hardware-family hints do not override this uncertainty."
                        )

                modem = context.get("modem") or {}
                if receiver_sensitivity_dbm is not None:
                    sensitivity = self._rf_validate_numeric(
                    ...
                    sensitivity = self._rf_receiver_sensitivity_dbm(
                        ...
                        (
                            int(modem.get("spreadingFactor"))
                            if modem.get("spreadingFactor")
                            else None
                        ),
                    )
                    if sensitivity is None:
                        raise ValueError(
                            "Receiver sensitivity could not be estimated from live modem settings. Provide receiver_sensitivity_dbm explicitly"
                        )
                    sensitivity_source = "thermal-noise estimate using 6 dB noise figure and LoRa SF threshold"
                    warnings.append(
                        "Receiver sensitivity is estimated; use a measured/module-specific value when available."
                    )
                ...
                budget.update(
                    {
                        ...
                        "distanceSource": (
                            ...
                            else "NodeDB endpoint positions"
                        ),
                        "frequencyMHz": round(frequency, 6),
                        "frequencySource": (
                            "explicit"
                            if explicit_frequency > 0
                            else context.get("frequencySource")
                        ),
                    }
                )
                ...
                if target:
                    observation = self._rf_observation_calibration(
                        target.get("snrDb"),
                        target.get("hopsAway"),
                        bool(target.get("viaMqtt")),
                        float(budget["predictedReceivePowerDbm"]),
                        self._rf_float(modem.get("bandwidthKHz")),
                    )
                ...
                endpoint_geometry = {
                    "local": {
                        "position": local_geo,
                        "antennaHeightMAboveGround": tx_height,
                        "antennaHeightSource": tx_height_source,
                    },
                    "remote": {
                        "position": remote_geo,
                        "antennaHeightMAboveGround": rx_height,
                        "antennaHeightSource": rx_height_source,
                    },
                }
                if include_terrain_profile:
                    if not (local_geo and remote_geo):
                        warnings.append(
                            "Terrain profile requested but both endpoint coordinates are not available."
                        )
                    else:
                        a = (
                            float(local_geo["latitude"]),
                        ...
                        terrain_used = True
                        local_ground_api = self._rf_float(
                            (terrain.get("points") or [{}])[0].get(
                                "groundElevationMAsl"
                            )
                        )
                        remote_ground_api = self._rf_float(
                            (terrain.get("points") or [{}, {}])[-1].get(
                                "groundElevationMAsl"
                            )
                        )
                        local_ground_explicit = (
                            float(self.valves.rf_local_ground_elevation_m_asl)
                            if self.valves.rf_use_local_installation_profile
                            and float(self.valves.rf_local_ground_elevation_m_asl)
                            > -9000.0
                            else None
                        )
                        remote_ground_explicit = self._rf_float(
                            remote_profile.get(
                                "ground_elevation_m_asl",
                                remote_profile.get("groundElevationMAsl"),
                            )
                        )
                        local_ant_asl, local_alt_source, local_ground = (
                            self._rf_endpoint_antenna_asl(
                                local_geo,
                                local_ground_api,
                                tx_height,
                                local_ground_explicit,
                        ...
                        remote_ant_asl, remote_alt_source, remote_ground = (
                            self._rf_endpoint_antenna_asl(
                                remote_geo,
                                remote_ground_api,
                                rx_height,
                                remote_ground_explicit,
                            )
                        )
                        endpoint_geometry["local"].update(
                            {
                                "groundElevationMAsl": local_ground,
                                "antennaElevationMAsl": local_ant_asl,
                                "antennaElevationSource": local_alt_source,
                            }
                        )
                        endpoint_geometry["remote"].update(
                            {
                                "groundElevationMAsl": remote_ground,
                                "antennaElevationMAsl": remote_ant_asl,
                                "antennaElevationSource": remote_alt_source,
                            }
                        )
                        if local_ant_asl is not None and remote_ant_asl is not None:
                            terrain_result = self._rf_terrain_analysis(
                                terrain,
                                distance,
                                frequency,
                                local_ant_asl,
                            ...
                            terrain_result["provider"] = {
                                k: v for k, v in terrain.items() if k != "points"
                            }
                        else:
                            warnings.append(
                                "Terrain elevations were fetched, but antenna ASL could not be established at both endpoints. Supply AGL heights or valid NodeDB device altitude."
                            )

                regulatory = self._rf_regulatory_assessment(
                    context, tx_power, tx_gain, tx_loss
                )
                if regulatory.get("status") == "exceeds_planner_limit":
                    warnings.append(
                        "The local antenna/cable/TX combination exceeds the planner's encoded radiated-power limit. Reduce TX power and verify current local rules."
                    )

                reverse = None
                remote_tx = remote_tx_power_dbm
                if remote_tx is None:
                    remote_tx = self._rf_float(
                        remote_profile.get(
                            "tx_power_dbm", remote_profile.get("txPowerDbm")
                        )
                    )
                if remote_tx is not None:
                    reverse_budget = self._rf_link_budget(
                        distance,
                        frequency,
                        float(remote_tx),
                        rx_gain,
                    ...
                    reverse = {
                        ...
                        "remoteTxPowerSource": (
                            "explicit"
                            if remote_tx_power_dbm is not None
                            else "known remote profile"
                        ),
                        "assumesSameModemSensitivityAtRemote": True,
                        "linkBudget": reverse_budget,
                    }

                local_profile_known = bool(
                    self.valves.rf_use_local_installation_profile
                    or any(v != 0.0 for v in (tx_gain, tx_loss, tx_height))
                )
                remote_profile_known = remote_profile.get("source") != "none" or any(
                    v != 0.0 for v in (rx_gain, rx_loss, rx_height)
                )
                confidence = self._rf_confidence(
                    terrain=bool(terrain_result),
                    local_profile=local_profile_known,
                    remote_profile=remote_profile_known,
                    actual_tx=actual_tx_known or actual_tx_power_dbm is not None,
                    direct_observation=observation is not None,
                )
                ...
                return {
                    ...
                    "simpleGeometry": self._rf_geometry(
                        distance, frequency, tx_height, rx_height
                    ),
    ...
    async def rf_compare_antennas(
        ...
        def run() -> Dict[str, Any]:
            ...
            if len(candidates) > 25:
                raise ValueError(
                    "A maximum of 25 antenna candidates can be compared at once"
                )

            baseline_gain, baseline_gain_source = self._rf_resolve_local_assumption(
                float(baseline_gain_dbi), "antennaGainDbi"
            )
            baseline_loss, baseline_loss_source = self._rf_resolve_local_assumption(
                float(baseline_cable_loss_db), "cableLossDb"
            )
            ...
            def op(iface: TCPInterface) -> Dict[str, Any]:
                ...
                if actual_tx_power_dbm is None:
                    overall_warnings.append(
                        "TX power is based on configuration/firmware limits, not measured hardware output."
                    )
                ...
                for index, item in enumerate(candidates):
                    ...
                    gain = self._rf_validate_numeric(
                        f"{name}.gain_dbi", item.get("gain_dbi", 0.0)
                    )
                    loss = self._rf_validate_numeric(
                        f"{name}.cable_loss_db", item.get("cable_loss_db", 0.0), 0.0
                    )
                    net = gain - loss
                    regulatory = self._rf_regulatory_assessment(
                        context, tx_power, gain, loss
                    )
                    row = dict(item)
                    row.update(
                        {
                    ...
                    if gain >= 8.0:
                        candidate_warnings.append(
                            "High-gain omni: verify the manufacturer's vertical radiation pattern; more gain is not automatically better for nearby or elevation-diverse nodes."
                        )
                    if regulatory.get("status") == "exceeds_planner_limit":
                        candidate_warnings.append(
                            "This candidate exceeds the encoded regional radiated-power planning limit at the assumed TX power."
                        )
    ...
    async def rf_plan_link(
        ...
        def run() -> Dict[str, Any]:
            ...
            rx_gain = self._rf_validate_numeric(
                "rx_antenna_gain_dbi", rx_antenna_gain_dbi
            )
            tx_loss, tx_loss_source = self._rf_resolve_local_assumption(
                float(tx_cable_loss_db), "cableLossDb"
            )
            rx_loss = self._rf_validate_numeric(
                "rx_cable_loss_db", rx_cable_loss_db, 0.0
            )
            tx_height, tx_height_source = self._rf_resolve_local_assumption(
                float(tx_height_m), "antennaHeightMAboveGround"
            )
            rx_height = self._rf_validate_numeric("rx_height_m", rx_height_m, 0.0)
            fade = self._rf_validate_numeric(
                "target_fade_margin_db", target_fade_margin_db, 0.0
            )
            extra_loss = self._rf_validate_numeric(
                "extra_path_loss_db", extra_path_loss_db, 0.0
            )
            ...
            def op(iface: TCPInterface) -> Dict[str, Any]:
                ...
                if actual_tx_power_dbm is not None:
                    ...
                    tx_power = self._rf_float(
                        (context.get("tx") or {}).get("planningTxPowerDbm")
                    )
                    if tx_power is None:
                        raise ValueError(
                            "TX power could not be derived. Provide actual_tx_power_dbm explicitly"
                    ...
                    tx_source = str(
                        (context.get("tx") or {}).get("planningTxPowerSource")
                        or "live configuration"
                    )
                    warnings.append(
                        "TX power is based on configuration/firmware limits rather than measured hardware output."
                    )
                ...
                if receiver_sensitivity_dbm is not None:
                    ...
                    if sensitivity is None:
                        raise ValueError(
                            "Receiver sensitivity could not be estimated. Provide receiver_sensitivity_dbm explicitly"
                        )
                    sensitivity_source = "thermal-noise estimate using 6 dB noise figure and LoRa SF SNR threshold"
                    warnings.append(
                        "Receiver sensitivity is estimated; a radio/module datasheet figure is preferable."
                    )
                ...
                max_distance = 10 ** (
                    (max_fspl - 32.44 - 20.0 * math.log10(frequency)) / 20.0
                )
                ...
                if regulatory.get("status") == "exceeds_planner_limit":
                    warnings.append(
                        "Current antenna/TX assumptions exceed the encoded regional radiated-power planning limit."
                    )
                ...
                return {
                    ...
                    "target": {
                        "distanceKm": round(distance, 3),
                        "targetFadeMarginDb": round(fade, 2),
                    },
                    "linkBudget": budget,
                    "geometry": self._rf_geometry(
                        distance, frequency, tx_height, rx_height
                    ),
                    "localInputSources": {
                        "antennaGain": tx_gain_source,
                        "cableLoss": tx_loss_source,
                        "antennaHeight": tx_height_source,
                    },
    ...
    async def rf_analyse_mesh(
        ...
        def run() -> Dict[str, Any]:
            ...
            if max_nodes < 1 or max_nodes > 500:
                raise ValueError("max_nodes must be between 1 and 500")
            remote_height_default = self._rf_validate_numeric(
                "assumed_remote_antenna_height_m", assumed_remote_antenna_height_m, 0.0
            )
            remote_gain_default = self._rf_validate_numeric(
                "assumed_remote_antenna_gain_dbi", assumed_remote_antenna_gain_dbi
            )
            remote_loss_default = self._rf_validate_numeric(
                "assumed_remote_cable_loss_db", assumed_remote_cable_loss_db, 0.0
            )
            fade = self._rf_validate_numeric("fade_margin_db", fade_margin_db, 0.0)
            ...
            def op(iface: TCPInterface) -> Dict[str, Any]:
                ...
                if not local_geo:
                    raise ValueError(
                        "Local node has no usable position; whole-mesh distance analysis requires a local position"
                    )
                local_pos = (
                    float(local_geo["latitude"]),
                    float(local_geo["longitude"]),
                ...
                if tx_power is None or frequency is None or sensitivity is None:
                    raise ValueError(
                        "Live radio frequency/TX/sensitivity context is incomplete"
                    )
                    ...
                    rows.append(
                        {
                            ...
                            "bearingDegrees": round(bearing, 1),
                            "bearingSector": self._rf_bearing_sector(bearing),
                            "reportedAltitudeAslM": remote_geo.get(
                                "reportedAltitudeAslM"
                            ),
                            "snrDb": entry.get("snr"),
                            "hopsAway": hops,
                            "viaMqtt": via_mqtt,
                            ...
                            "knownRemoteProfile": (
                                profile if profile.get("source") != "none" else None
                            ),
                            "remoteInputSources": {
                                "gain": rx_gain_source,
                                "loss": rx_loss_source,
                                "height": rx_height_source,
                            },
                            "simpleGeometry": self._rf_geometry(
                                distance, frequency, tx_height, rx_height
                            ),
                            "freeSpaceMarginAfterFadeDb": budget[
                                "marginAfterFadeAllowanceDb"
                            ],
                            "category": category,
                            "nextBestCheck": (
                                "Run rf_analyse_link for this node with include_terrain_profile=True"
                                if category.startswith("multi_hop")
                                else None
                            ),
                        }
                    )
                ...
                rows.sort(
                    key=lambda r: (
                        0 if r["category"] == "direct_observed" else 1,
                        r["distanceKm"],
                    )
                )
                rows = rows[:max_nodes]
                excess_values = sorted(
                    float(c["estimatedExcessPathLossVsFreeSpaceDb"])
                ...
                if excess_values:
                    ...
                    median_excess = (
                        excess_values[n // 2]
                        if n % 2
                        else (excess_values[n // 2 - 1] + excess_values[n // 2]) / 2.0
                    )
                sectors: Dict[str, List[float]] = {}
                for c in calibrations:
                    sectors.setdefault(str(c["bearingSector"]), []).append(
                        float(c["estimatedExcessPathLossVsFreeSpaceDb"])
                    )
                sector_summary = {
                    k: round(sum(v) / len(v), 2) for k, v in sectors.items()
                }
                ...
                return {
                    ...
                    "localRfChain": {
                        "antennaGainDbi": tx_gain,
                        "gainSource": tx_gain_source,
                        "cableLossDb": tx_loss,
                        "lossSource": tx_loss_source,
                    },
                    "windowMinutes": active_within_minutes,
                    ...
                    "directLinkCalibration": {
                        ...
                        "medianEstimatedExcessLossVsFreeSpaceDb": (
                            round(median_excess, 2)
        ...
        return await self._run_sync(
            run,
            __event_emitter__,
            "Analysing RF characteristics of the local mesh…",
            "Whole-mesh RF analysis complete",
        )
    ...
    async def get_config(self, section: str = "all", __event_emitter__=None) -> str:
        ...
        return await self._run_sync(
            run,
            __event_emitter__,
            "Reading Meshtastic configuration…",
    ...
    async def get_config_schema(self, section: str, __event_emitter__=None) -> str:
        ...
        return await self._run_sync(
            run, __event_emitter__, "Inspecting config schema…", "Config schema loaded"
        )

    async def get_channels(
        self, include_disabled: bool = False, __event_emitter__=None
    ) -> str:
        ...
        return await self._run_sync(
            run, __event_emitter__, "Reading Meshtastic channels…", "Channels loaded"
        )

    async def get_channel_schema(
        self, channel_index: int = 0, __event_emitter__=None
    ) -> str:
        ...
        return await self._run_sync(
            run,
            __event_emitter__,
            "Inspecting channel schema…",
            "Channel schema loaded",
        )

    async def get_share_url(
        self, include_all_channels: bool = False, __event_emitter__=None
    ) -> str:
        """
        ...
        return await self._run_sync(
            run,
            __event_emitter__,
            "Generating Meshtastic share URL…",
            "Share URL generated",
        )
    ...
    async def request_telemetry(
        ...
        def run() -> Dict[str, Any]:
            ...
            ch = (
                self.valves.default_channel_index
                if channel_index < 0
                else channel_index
            )
            if ch < 0 or ch > 7:
            ...
            def op(iface: TCPInterface) -> Dict[str, Any]:
                ...
                def on_response(packet: Dict[str, Any]) -> None:
                    ...
                    if (
                        ...
                        or portnum == portnums_pb2.PortNum.TELEMETRY_APP
                    ):
            ...
            def op(iface: TCPInterface) -> Dict[str, Any]:
                ...
                def on_response(packet: Dict[str, Any]) -> None:
                    ...
                    if (
                        ...
                        or portnum == portnums_pb2.PortNum.POSITION_APP
                    ):
    ...
    async def send_message(
        ...
        if not self.valves.allow_messages:
            return self._json(
                {"ok": False, "error": "Messaging is disabled in the tool Valves"}
            )
        ...
        return await self._run_sync(
            run, __event_emitter__, "Sending Meshtastic message…", "Message sent"
        )
    ...
    async def send_alert(
        ...
        if not self.valves.allow_messages or not self.valves.allow_alerts:
            return self._json(
                {
                    "ok": False,
                    "error": "High-priority alerts are disabled in the tool Valves",
                }
            )
        encoded = message.encode("utf-8")
            ...
            return self._json(
                {
                    "ok": False,
                    "cancelled": True,
                    "error": "Alert send was not confirmed",
                }
            )
        ...
        return await self._run_sync(
            run, __event_emitter__, "Sending Meshtastic alert…", "Alert sent"
        )
            ...
            return self._json(
                {
                    "ok": False,
                    "cancelled": True,
                    "error": "Configuration change was not confirmed",
                }
            )
    ...
    async def preview_config_batch(
        self, changes_json: str, __event_emitter__=None
    ) -> str:
        """
        ...
        def run() -> Dict[str, Any]:
            ...
            def op(iface: TCPInterface) -> Dict[str, Any]:
                ...
                for change in changes:
                    ...
                    proposal = self._set_proto_value(
                        message, change["field"], change["value"]
                    )
        ...
        return await self._run_sync(
            run,
            __event_emitter__,
            "Validating config batch…",
            "Config batch preview ready",
        )
            ...
            return self._json(
                {
                    "ok": False,
                    "cancelled": True,
                    "error": "Configuration batch was not confirmed",
                }
            )
    ...
    async def set_node_name(
        ...
        if not self.valves.allow_config_writes:
            return self._json(
                {"ok": False, "error": "Configuration writes are disabled"}
            )
        ...
        def run() -> Dict[str, Any]:
            def op(iface: TCPInterface) -> Dict[str, Any]:
                ...
                time.sleep(0.2)
                return {
                    "longName": long_name.strip(),
                    "shortName": short_name.strip()[:4],
                }

            return self._with_interface(op)

        return await self._run_sync(
            run, __event_emitter__, "Renaming Meshtastic node…", "Node renamed"
        )
    ...
    async def set_channel_value(
        ...
        if (
            self._is_sensitive_key(field)
            and not self.valves.allow_sensitive_config_writes
        ):
            return self._json(
                {"ok": False, "error": "Sensitive channel writes are disabled"}
            )
        ...
        return await self._run_sync(
            run, __event_emitter__, "Writing channel settings…", "Channel updated"
        )
    ...
    async def set_channel_psk(
        ...
        """
        if (
            not self.valves.allow_channel_writes
            or not self.valves.allow_sensitive_config_writes
        ):
            return self._json(
                {"ok": False, "error": "Sensitive channel writes are disabled"}
            )
        ...
        return await self._run_sync(
            run, __event_emitter__, "Updating channel PSK…", "Channel PSK updated"
        )
    ...
    async def delete_channel(
        ...
        if channel_index <= 0 or channel_index > 7:
            return self._json(
                {
                    "ok": False,
                    "error": "Only secondary channel indexes 1-7 may be deleted",
                }
            )
        ...
        return await self._run_sync(
            run, __event_emitter__, "Deleting Meshtastic channel…", "Channel deleted"
        )
    ...
    async def import_channel_url(
        ...
        confirmed = await self._confirm(
            ...
            + (
                "Existing channels will be preserved where possible."
                if add_only
                else "This can replace existing channel/radio settings."
            ),
        )
        if not confirmed:
            return self._json({"ok": False, "cancelled": True})
        ...
        return await self._run_sync(
            run,
            __event_emitter__,
            "Importing channel configuration…",
            "Channel configuration imported",
        )
    ...
    async def set_node_favorite(
        ...
        return await self._run_sync(
            run,
            __event_emitter__,
            "Updating NodeDB favourite…",
            "NodeDB favourite updated",
        )
    ...
    async def remove_node_from_database(
        ...
        return await self._run_sync(
            run,
            __event_emitter__,
            "Removing node from NodeDB…",
            "Node removed from NodeDB",
        )

    async def reset_node_database(
        self, __event_call__=None, __event_emitter__=None
    ) -> str:
        ...
        return await self._run_sync(
            lambda: self._with_interface(
                lambda iface: (iface.localNode.resetNodeDb(), {"reset": True})[1]
            ),
            __event_emitter__,
    ...
    async def set_fixed_position(
        ...
        if latitude < -90 or latitude > 90 or longitude < -180 or longitude > 180:
            return self._json(
                {"ok": False, "error": "Latitude/longitude are outside valid ranges"}
            )
        ...
        return await self._run_sync(
            run,
            __event_emitter__,
            "Setting fixed Meshtastic position…",
            "Fixed position updated",
        )

    async def remove_fixed_position(
        self, __event_call__=None, __event_emitter__=None
    ) -> str:
    ...
    async def sync_device_time(
        ...
        if not self.valves.allow_admin_actions:
            return self._json(
                {"ok": False, "error": "Administrative actions are disabled"}
            )
        now = int(time.time())
        confirmed = await self._confirm(
            __event_call__,
        ...
        return await self._run_sync(
            run,
            __event_emitter__,
            "Synchronising Meshtastic clock…",
            "Clock synchronised",
        )
    ...
    async def reboot_device(
        ...
        if delay_seconds < 1 or delay_seconds > 60:
            return self._json(
                {"ok": False, "error": "delay_seconds must be between 1 and 60"}
            )
        confirmed = await self._confirm(
            __event_call__,
        ...
        return await self._run_sync(
            lambda: self._with_interface(
                lambda iface: (
                    iface.localNode.reboot(delay_seconds),
                    {"rebootScheduledInSeconds": delay_seconds},
                )[1]
            ),
            __event_emitter__,
    ...
    async def shutdown_device(
        ...
        return await self._run_sync(
            lambda: self._with_interface(
                lambda iface: (
                    iface.localNode.shutdown(delay_seconds),
                    {"shutdownScheduledInSeconds": delay_seconds},
                )[1]
            ),
            __event_emitter__,
